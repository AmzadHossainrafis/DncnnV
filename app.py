...
model.load_state_dict(
    torch.load(prediction_config["model_path"], map_location=torch.device("cpu"))
)
model.eval()
...
prediction_transform = A.Compose(
    [
        ...
        A.Resize(
            prediction_config["transform"]["image_size"],
            prediction_config["transform"]["image_size"],
            p=1,
        ),
        ToTensorV2(),
        ...
        if file and allowed_file(file.filename):
            try:
                ...
                converted_image.save(
                    os.path.join(app.config["RESTORED_FOLDER"], filename)
                )
                ...
                return render_template(
                    "error.html", error="Something went wrong. Please try again later."
                )
...
def get_prediction(image):
    ...
    with torch.no_grad():
        ...
        preds = denormalize(
            preds.squeeze(0).squeeze(0).numpy(),
            prediction_config["transform"]["normalization"]["mean"],
            prediction_config["transform"]["normalization"]["std"],
        )
