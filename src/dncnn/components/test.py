...
val_dataloader = DataLoader(hr_dir, batch_size=16, shuffle=True, num_workers=4, transform=True) 
...
def plot_val_data(val_dataloader):
    model = DnCNN().to("cuda")
    model.load_state_dict(torch.load(r"C:\Users\Amzad\Desktop\Dncnn\artifact\model_ckpt\model.pth"))
    model.eval()
...
def single_prediction(img_dir):
    model = DnCNN().to("cuda")
    model.load_state_dict(torch.load(r"C:\Users\Amzad\Desktop\Dncnn\artifact\model_ckpt\model_mv2-100.pth"))
    model.eval()
    ...
    # axarr[1].imshow(img[0].permute(1,2,0).numpy().astype(np.uint8))
    # axarr[0].imshow(sr[0].permute(1,2,0).numpy().astype(np.uint8))
...
if __name__ == "__main__":
    ...
    single_prediction(r'G:\s\val\hr/1.jpg')
